...
if [ ! -z "$TRAVIS_TAG" ] ; then
   if [[ (! -z "$DOCKER_USERNAME") &&  (! -z "$DOCKER_PASSWORD") ]]; then
     ...
     echo "[INFO] The travis build is for TRAVIS_TAG=$TRAVIS_TAG but DOCKER_USERNAME and/or DOCKER_PASSWORD are not provided, skipping the build and publish of the utils image." 
   fi
else
